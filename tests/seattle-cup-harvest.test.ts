...
test('migration uses a versioned validated evidence envelope and remains private and append-only', () => {
  ...
  assert.match(sql, /REVOKE ALL ON public\.scouting_reports FROM PUBLIC, anon, authenticated, service_role/)
  assert.match(sql, /GRANT SELECT ON public\.scouting_reports TO authenticated/)
  assert.match(sql, /GRANT SELECT, INSERT ON public\.scouting_reports TO service_role/)
