...
FAST integration is the default. Treat `fast integrate` after a READY report as the command to begin delivery immediately. Follow [AGENTS.md](AGENTS.md) for the concise execution contract and [docs/DELIVERY.md](docs/DELIVERY.md) for the full FAST/GUARDED policy.

Repo-specific deployment facts:

- GitHub repository `libreninja/planit-golf`, branch `main`, is the production source. Squash-merging a PR to `main` starts the Vercel deployment.
- Tracked [`.vercel/repo.json`](.vercel/repo.json) is the identity authority: project `nextjs-boilerplate`, ID `prj_jYKeHFnoeXhi30uhvtjAD1H1jQL2`, org `team_2OwWZEiWXYEgSrbbyaqfrRNg`.
- Production domains are `www.planit.golf`, `planit.golf`, and `bigdeal.planit.golf`.
- Production must originate from the GitHub integration. Never use `vercel deploy --prod` or `vercel deploy --target=production`; CLI-originated production deployments have served global 500s despite reporting READY.
- `pnpm integration:deployment -- <merged-main-sha>` uses exact-SHA GitHub deployment metadata. Vercel CLI/MCP listings can omit Git-triggered deployments and are not authoritative evidence of absence.
- Production environment variables live in the Vercel project. Preview `*.vercel.app` URLs are SSO-protected; custom production domains are public.
- Rollback uses the last known-good GitHub-`main` production deployment. Establish its exact provenance before promotion.
- `pnpm build` typechecks the application build graph, not every standalone test or unimported file.
